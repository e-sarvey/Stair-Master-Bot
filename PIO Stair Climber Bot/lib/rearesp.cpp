...
// ------------ // MAIN FUNCTIONS // ------------ //
void move_one_stair(){
    // front drives to step
    // stops when lidar threshold met and sends message
    await_message("front", "stair_reached");
    moveStepper(FRONT_LIFT_STEPS);
    moveForward(FORWARD_TIME); // do we have another way to detect this?
    moveStepper(FRONT_LOWER_STEPS); // lower onto steps
    update_status("rear", "front_placed");
    // front moves mass to prepare for rear lifing
    await_message("front", "mass_moved");
    moveStepper(BACK_LIFT_STEPS);
    update_status("rear", "rear_lifted");
